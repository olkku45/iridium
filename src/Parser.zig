...
pub const AstPrinterTT = struct {
    ...
    fn traverseStmt(stmt: Stmt) void {
        switch (stmt) {
            .if_stmt => |s| {
                print("if statement | ", .{});
                print("condition: \n", .{});
            ...
            .extern_fn_decl => |s| {
                print("extern fn name: {s}, ", .{s.name.literal.value});
                print("arg type: {s}, ", .{s.arg_type.primitive});
                print("ret type: {s}\n", .{s.ret_type.primitive});
            },
            .fn_decl => |s| {
                print("fn name: {s}, ", .{s.name.literal.value});
                print("ret type: {s}, ", .{s.ret_type.primitive});
                print("fn body: \n", .{});
                for (s.fn_body) |item| {
                    indent();
                    traverseStmt(item);
            ...
            .var_decl => |s| {
                print("var name: {s}, ", .{s.name.literal.value});
                print("var type: {s}, ", .{s.var_type.primitive});
                print("var value: {any}\n", .{s.value});
            },
            .expr_stmt => |s| {
                traverseExpr(s.expr);
            ...
            .ret_stmt => |s| {
                print("ret stmt value: {any}\n", .{s.value});
            }
        }
    }

    fn traverseExpr(expr: Expr) void {
        switch (expr) {
            .binary => |e| {
                indent();
                indent();
                print("bin expr left: {s}, ", .{e.*.left.literal.value});
                print("bin op: {any}, ", .{e.*.op});
                print("bin expr right: {s}\n", .{e.*.right.literal.value});
            },
            .func_call => |e| {
                print("called fn name: {s}\n", .{e.*.func_name.literal.value});
                for (e.*.args) |arg| {
                    indent();
                    print("called fn arg: {s}\n", .{arg.literal.value});
                }
            },
            else => {},
        }
