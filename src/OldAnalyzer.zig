...
pub const Analyzer = struct {
        ...
        for (func.fn_body) |stmt| {
            switch (stmt) {
                ...
                .var_decl => |s| {
                    ...
                    //stmt.var_decl.symbol = decl; TODO: this doesn't work
                },
                .if_stmt => |s| {
    ...
    fn checkLiteral(self: *Analyzer, expr: Expr.Literal) !LiteralType {
        switch (expr.type) {
            .identifier => {
                if (self.symbol_table.lookupItem(expr.value) == null) {
                    reportLiteralError(expr);
                    return AnalysisError.VariableOutOfScope;
                } else return LiteralType.identifier;
            },
            .int => {},
            .float => {},
            .boolean => {},
    ...
    fn checkBinExpr(self: *Analyzer, expr: Expr.BinaryExpr) !void {
        // assuming left and right are literals here
        const left_type = try checkLiteral(expr.left.literal);
        const right_type = try checkLiteral(expr.right.literal);

        if (left_type == .identifier and right_type == .identifier) {
            const left_s = self.symbol_table.lookupItem(expr.left.literal.value);
            const right_s = self.symbol_table.lookupItem(expr.right.literal.value);

            if (left_s.?.data_type.named.primitive != right_s.?.data_type.named.primitive) {
    ...
    fn checkCondition(self: *Analyzer, expr: Expr) !void {
        switch (expr) {
            .literal => |lit| {
                try checkLiteral(self, lit);
            },
            .binary => |bin| {
                try checkBinExpr(self, bin);
            },
            else => {},
        }
    }

    fn checkWhileLoop(self: *Analyzer, stmt: Stmt.WhileLoop) !void {
        try checkCondition(self, stmt.cond);
    ...
    // TODO: check that the expression even evaluates to a boolean
